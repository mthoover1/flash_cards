class Guess < ActiveRecord::Base
  ...
  def check_answer
    ...
    unless @deck.id == 1
      ...
      proximity = (self.response.to_i - Card.find(card_id).solution.to_i).abs

      self.correctness = 5 if proximity == 0
      self.correctness = 3 if proximity == 1
      self.correctness = 2 if proximity == 2
      self.correctness = 1 if proximity == 3
    end
  end
